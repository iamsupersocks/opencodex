...
| --- | --- | --- | --- |
| `xai` | `openai-chat` | `https://cli-chat-proxy.grok.com/v1` | OAuth uses the separate Grok CLI subscription gateway. Fast is verified only for OAuth `grok-4.6` on `/v1/responses`. The API-key override uses `https://api.x.ai/v1` and may inject Priority Processing on the public API. Live-first Grok catalog; `grok-4.5` is the fallback default. |
